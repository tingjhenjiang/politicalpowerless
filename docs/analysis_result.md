...
date: 'First created on Jan 15, 2019, 09:00. Last updated on 2019-01-16 00:38:13'
output:
  html_document:
    keep_md: yes
    toc: true
...
* 統獨傾向：依據問卷中的以下問題類別變項，使用以潛在類別模式（latent class model）為原理的R語言套件poLCA{Linzer, 2011 #12918}，並以下述政黨傾向外顯變項為共變項，預測／估計受訪者潛在的統獨傾向，分為「統一」、「中立」、「獨立」三類。括弧內為潛在類別模式估計外顯類別對潛在類別的機率；2010環境並無統獨傾向問題所以並未建立變項、2016公民並不使用潛在類別模式估計，直接以題目回答結果編碼：
<table class="table table-striped table-hover" style="margin-left: auto; margin-right: auto;">
 <thead>
  <tr>
   <th style="text-align:left;">   </th>
...
* 政黨傾向：依據問卷中的以下問題類別變項，使用以潛在類別模式（latent class model）為原理的R語言套件poLCA{Linzer, 2011 #12918}，並以前述統獨傾向外顯變項為共變項，預測／估計受訪者潛在的政黨傾向，分為「泛藍」、「中立」、「泛綠」三類。括弧內為潛在類別模式估計外顯類別對潛在類別的機率；2010環境並不使用潛在類別模式估計，直接以題目回答結果編碼：
<table class="table table-striped table-hover" style="margin-left: auto; margin-right: auto;">
 <thead>
  <tr>
   <th style="text-align:left;">   </th>
