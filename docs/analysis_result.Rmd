...
接著，選票的獲得並不是只來自選民對於議員的政策回應而來。在探討何謂回應性的這個概念過程中，提出其中共有四個子概念，分別是政策回應、分配回應、the target is the great public issues that agitate the political process. Second, there is service responsiveness which involves the efforts of the representative to secure particularized benefits for individuals or groups in his constituency. Third, there is alloca- tion responsiveness which refers to the representative's efforts to obtain benefits for his constituency through pork-barrel exchanges in the appropri- ations process or through administrative interventions. Finally, there is what we shall call symbolic responsiveness which involves public gestures of a sort that create a sense of trust and support in the relationship between representative and represent。從而，政策回應只是代表的其中一個面向{Eulau, 1989 #12909}。或者選民特性使然──例如，選民傾向以議員的服務回應或分配回應評估候選人{Griffin, 2010 #12873}，因此其重要性超越政策回應／象徵代表時──
...
model_indp_var <- c("myown_selfid", "myown_factoredses", "myown_factoredparticip","gap_sex", "gap_eduyr", "gap_ses", "gap_ethnicity", "gap_age", "days_diff_survey_bill","elec_dist_type","legislator_party","incumbent","pp_agendavoting","pp_lawamendment", "seniority")
...
if ({ordered_logistic_to_analy_repre_response<-FALSE; ordered_logistic_to_analy_repre_response}) {
  models_respondopinion_by_district_legislator <- lapply(overall_district_legislators_only_power_dfdata,function(dataset_for_analysis,...) {
  ...
  #exportvar=c("overall_district_legislators_only_power_dfdata","respondopinion_by_district_legislator_formula")
  )
}
save(models_respondopinion_by_district_legislator,file=paste0(dataset_in_scriptsfile_directory, "models_respondopinion_by_district_legislator_stepped.RData"))
```

```{r models_respondopinion_by_district_legislator_analyse, echo=FALSE}
#viftestresult <- lapply(models_respondopinion_by_district_legislator,car::vif)

result_repre_response_on_all_bills_table <- custom_parallel_lapply(models_respondopinion_by_district_legislator,function(model_respondopinion_by_district_legislator,...) {
