...
respondmodel_args<-data.frame("formula"=c(
  ...
  "respondopinion~1+days_diff_survey_bill_overallscaled*myown_factoredparticip_overallscaled+(days_diff_survey_bill_overallscaled|admindistrict/id_wth_survey)+(1|billid_myown)+issuefield+myown_factoredses_overallscaled+myown_marriage+myown_areakind+myown_age_overallscaled+myown_sex+myown_selfid+myown_religion+similarity_distance_overallscaled*myown_factoredparticip_overallscaled+(1|admindistrict/id_wth_survey)+cluster_kamila*myown_factoredparticip_overallscaled+elec_dist_type+seniority_overallscaled+(1|partyGroup/legislator_name)+party_pressure_overallscaled+partysize+adminparty+SURVEY",
  ...
  dplyr::mutate(storekey=paste0(c("crtiny","null"),withindays)) %>%
