source(file = "03_preprocessing_legislators_and_elections.R")
survey_adding_area_settingNA_class <- R6::R6Class("survey_adding_area_settingNA", inherit=legislators_and_elections_parser_class, public = list(
  survey_imputation_and_measurement = NULL,
  initialize = function(dataset_in_scriptsfile_directory="/mnt", filespath="/mnt", dataset_file_directory="/mnt", debug_func_mode=TRUE) {
    ...
    self$survey_imputation_and_measurement<-self$custom_ret_survey_imputation_and_measurement(self$paths_to_survey_imputation_and_measurement_file)
  },
  # 第三部份：把問卷檔加上行政區、選區屬性  -------------------------------------------
  #以下不處理的時候會直接跳到讀取rdata
  processing_duplicated_area = function(loading=FALSE, saving=TRUE) {
    if (loading==TRUE) {
    ...
    admin_dist_to_elect_dist <- dplyr::distinct(elections_df, term, admincity, electionarea, admindistrict, adminvillage) %>%
      dplyr::filter(!is.na(admincity))# %>%
    ##  left_join(all_admin_dist_with_zip)
    duplicated_area <- dplyr::distinct(admin_dist_to_elect_dist,term,electionarea,admincity,admindistrict) %>% #,zip,zip3rocyear
      magrittr::extract(duplicated(.[, c("term", "admincity", "admindistrict")]),)
    #把某些共用同一個郵遞區號的行政區合併
    unique_dist_for_elect_dist <- dplyr::anti_join(admin_dist_to_elect_dist, duplicated_area[, c("term", "admincity", "admindistrict")]) %>%
      dplyr::group_by(term, electionarea, admincity) %>% #, zip, zip3rocyear
      dplyr::summarise(admindistrict = paste0(unique(admindistrict), collapse = "、")) %>%
      dplyr::arrange(term, electionarea, admincity, admindistrict)
    unique_electionarea_for_all_admindist <- dplyr::distinct(admin_dist_to_elect_dist, term, electionarea, admincity, admindistrict) %>%
      dplyr::group_by(term, electionarea) %>%
      dplyr::summarise(admincity = paste0(unique(admincity), collapse="、"), admindistrict = paste0(unique(admindistrict), collapse = "、")) %>%
      dplyr::arrange(term, admincity, electionarea, admindistrict)
    #以下註解部分為找出多選區的樣本
    ...
    return(list(
      "admin_dist_to_elect_dist"=admin_dist_to_elect_dist,
      "duplicated_area"=duplicated_area,
      "unique_dist_for_elect_dist"=unique_dist_for_elect_dist,
      "unique_electionarea_for_all_admindist"=unique_electionarea_for_all_admindist
    ))
  },
  otherobjs = function() {
    ...
    survey_data<-paste0(survey_data_title,".sav") %>%
      sapply(function (X,...) file.path(..., X), dataset_file_directory, "merger_survey_dataset") %>%
