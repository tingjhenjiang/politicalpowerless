...
merge_votingrecords_surveyquestions_class <- R6::R6Class("merge_votingrecords_surveyquestions", inherit=base_r6_class, public = list(
  adminparty = list(
    data.frame(
      "term"=5,
    ...
    data.frame(
      "term"=7,
      "party"=c("中國國民黨","民主進步黨","無黨團結聯盟","親民黨","無黨籍及未經政黨推薦","無黨籍"),
      "adminparty"=c(1,0,0,0,0,0)
    ),
    data.frame(
      "term"=8,
  ...
  non_decision = "棄權/未投票/請假/缺席",
  non_decision_vec = NULL,
  ...
  initialize = function(dataset_in_scriptsfile_directory="/mnt", filespath="/mnt", dataset_file_directory="/mnt", debug_func_mode=TRUE) {
    ...
    self$survey_time_range_df <- dplyr::bind_rows(self$survey_time_range)
    self$non_decision_vec <- unlist(strsplit(self$non_decision,"/",fixed=TRUE))
  },
  # （通常略過）讀取投票紀錄資料(此處通常預處理好，直接load下面 mergedf_votes_bills_surveyanswer)  -------------------------------------------
  merging_terms_56789 = function(loading=TRUE, saving=TRUE) {
    if (loading==TRUE) {
      return (
        readRDS(file=self$myown_vote_record_df_across2004_filepath)
      )
    }
    ...
    currentEnv$non_decision <- self$non_decision
    currentEnv$non_decision_vec <- self$non_decision_vec
    ...
    myown_vote_record_df <- dplyr::arrange(myown_vote_record_df, term, period, temp_meeting_no, meetingno, billn) %>%
      dplyr::mutate_at(c("billresult"), as.factor) %>%
    ...
    # browser()
    myown_vote_record_df <- mutate_cond(myown_vote_record_df, votedecision %in% non_decision_vec, votedecision=non_decision, envir=currentEnv) %>% 
      dplyr::mutate_at(c("votedecision"), as.factor)

    if (saving) {
      saveRDS(myown_vote_record_df,file=self$myown_vote_record_df_across2004_filepath)
    }
    return (myown_vote_record_df)
  },
  # （略過）把投票紀錄整理輸出成議案紀錄 -------------
  writing_bill_record = function(filename="TMP.xlsx", loading=TRUE) {
    myown_vote_record_df <- self$merging_terms_56789(loading=loading)
    dplyr::distinct(myown_vote_record_df, url, billcontent, term, period, temp_meeting_no, meetingno, billn, billresult, date, billid_myown) %>%
      openxlsx::write.xlsx(filename)
  },
  # 將合併好的投票紀錄進一步處理  -------------------------------------------
  overall_futher_preprocess_merged_dataset = function(loading=TRUE) {
    # load(file=paste0(dataset_in_scriptsfile_directory, "myown_vote_record_df_across2004.RData"), verbose=TRUE)
    myown_vote_record_df_across2004 <- self$merging_terms_56789(loading=loading)
    # load(file=paste0(dataset_in_scriptsfile_directory, "legislators_with_elections.RData"), verbose=TRUE)
    legislators_with_elections <- readRDS(file=self$legislators_with_elections_filepath)

    tryCatch({
      bills_answer_to_bill <- openxlsx::read.xlsx(self$myown_vote_bills_filepath, sheet = 4)
      bills_billcontent <- openxlsx::read.xlsx(self$myown_vote_bills_filepath, sheet = 1) %>%
        dplyr::mutate_at("billcontent", as.character) %>%
        dplyr::mutate_at("pp_agendavoting", as.factor)
      # browser()
      # base::setdiff(myown_vote_record_df_across2004$billid_myown, bills_billcontent$billid_myown)
      # base::setdiff(bills_billcontent$billid_myown, myown_vote_record_df_across2004$billid_myown)
      ...
      return(list(
        "bills_answer_to_bill"=bills_answer_to_bill,
        "bills_billcontent"=bills_billcontent,
        "bills_billcontent_with_relatedq"=bills_billcontent_with_relatedq,
        "term_related_q"=term_related_q,
        "bills_billid_to_relatedq_pairs"=bills_billid_to_relatedq_pairs
      ))
    }, error = function(msg) {
      message(paste0(msg,"\n"))
      browser()
  ...
  # 測試有無重複投票紀錄  -------------------------------------------
  testing_if_duplicated = function(loading=TRUE) {
    myown_vote_record_df <- self$merging_terms_56789(loading=loading)
    browser()
  ...
  compose_dataset_for_factor_analysis_of_voterecords = function() {
    browser()
    ...
    widedata_formula <- paste0(widedata_preserve_vars, collapse="+") %>%
      paste0("~","billid_myown") %>%
      as.formula()
    newlevel_of_voting<-c("反對", self$non_decision, "贊成")
  ...
  detect_ncomponents_parallel_analysis = function() {
    browser()
  ...
  efa_voterecords_mirt_mixedeffect = function() {
    browser()
  ...
  # * EFA by IRT.fa --------------------------------
  efa_voterecords_irt_fa = function() {
    browser()
    res.irtefas <- list()
  ...
  efa_voterecords_polychoric_correlation = function() {
    browser()
  ...
  # * EFA by homals --------------------------------
  efa_voterecords_homals = function() {
    browser()
    res.homals <- list()
  ...
  # * EFA by gifi homels --------------------------------
  efa_voterecords_gifi_homels = function() {
  ...
  # 第三部分：立法委員資料與投票資料合併  -------------------------------------------
  merge_legislators_voterecords = function(loading=TRUE) {
    # browser()
    myown_vote_record_df <- self$merging_terms_56789(loading=loading)
    legislators_with_elections <- readRDS(file=self$legislators_with_elections_filepath)
    terms <- self$terms
    ...
    mergedf_votes_bills_surveyanswer <- dplyr::distinct(legislators_with_elections,term,legislator_name,legislator_party,legislator_sex,seniority,legislator_age,incumbent,elec_dist_type) %>%
      ...
      dplyr::left_join(dplyr::filter(as.data.frame(myown_vote_record_df), term %in% self$terms), .) %>%
      ...
      dplyr::left_join(., { #此處設計一個政黨壓力指標並串連加入
        dplyr::group_by(., votedecision, billid_myown, legislator_party) %>%
          dplyr::summarise(samepartysamepositioncounts=dplyr::n()) %>%
        ...
        dplyr::left_join(self$adminparty, by=c("term"="term","legislator_party"="party")) %>%
        dplyr::filter(!is.na(legislator_name))
    return(mergedf_votes_bills_surveyanswer)
  },
  
  # 第四部分：合併投票紀錄與法案資料  -------------------------------------------
  merge_voterecords_billcontents = function(loading=TRUE) {
    # browser()
    currentEnv <- environment()
    currentEnv$non_decision <- self$non_decision
    currentEnv$non_decision_vec <- self$non_decision_vec
    overall_futher_preprocess_merged_dataset_results <- self$overall_futher_preprocess_merged_dataset()
    mergedf_votes_bills_surveyanswer <- self$merge_legislators_voterecords(loading=loading)
    mergedf_votes_bills_surveyanswer <- mergedf_votes_bills_surveyanswer %>%
      dplyr::left_join(overall_futher_preprocess_merged_dataset_results$bills_billcontent, by = c("billid_myown","term","period","meetingno","temp_meeting_no","billn","billresult","date")) %>% ##,"url"
      dplyr::select(-tidyselect::contains("billarea0")) %>%
      dplyr::right_join(overall_futher_preprocess_merged_dataset_results$bills_answer_to_bill, by = c("billid_myown")) %>%  #left 1419337 right 926609
      #篩選出研究範圍
      ...
      mutate_cond(votedecision=="反對", opiniondirectionfromlegislator=dplyr::recode(
      ...
      mutate_cond(votedecision==non_decision, respondopinion=1, opiniondirectionfromlegislator='ig/gu', envir=currentEnv) %>% #ignore/giveup
