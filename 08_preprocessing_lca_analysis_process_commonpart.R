source(file = "07_preprocessing_fa_and_irt_process.R")
# 第六部份：LCA latent variables 潛在類別模式共用部分 -------------------------------------------
#自動變數選擇https://cran.r-project.org/web/packages/LCAvarsel/LCAvarsel.pdf
lca_analysis_commonpart_class <- R6::R6Class("lca_analysis_commonpart", inherit=fa_and_irt_process_class, public = list(
  lcaneed_independence_attitude = NULL,
  lcaneed_party_constituency = NULL,
  lcaneed_ethnicity = NULL,
  lcaneed_identity = NULL,
  lcaneed_other_cov = NULL,
  imps = NULL,
  detectedcores = NULL,
  dbconnect_info = NULL,
  initialize = function(dataset_in_scriptsfile_directory="/mnt", filespath="/mnt", dataset_file_directory="/mnt", debug_func_mode=TRUE) {
    ...
    self$lcaneed_identity<-list(
      "2004citizen"=c("v94r"),
      "2010env"=c(),
      "2010overall"=c("v89r"),
      "2016citizen"=c() #到這裡，但還沒有輸出上述的v94r
    )
    self$lcaneed_other_cov<-list(
    ...
    self$imps <- imputation_sample_i_s
    self$detectedcores <- ifelse(check_if_windows(),1,parallel::detectCores())
  },
  # 建立資料庫連線資訊（記錄LCA model search進度用）
  build_dbconnect_info = function(dbpassword=NULL, dbhost="tjhome.crabdance.com", dbname="thesis", dbusername="j", dbport=3306, dbtype=RMariaDB::MariaDB()) {
    if (is.null(dbpassword)) {
      dbpassword <- askpass::askpass(prompt = "Please enter your password: ")#rstudioapi::askForPassword("input password")
    }
    self$dbconnect_info <- list(
      "drv"=dbtype,
      "host"=dbhost,
      "dbname"=dbname,
      "username"=dbusername,
    ...
    #測試連線
    con <- do.call(DBI::dbConnect, self$dbconnect_info)
    DBI::dbDisconnect(con)
    invisible(self$dbconnect_info)
  },
  # 檢查／安裝external/poLCA子模組版（支援sample weights之weighted pseudo-ML）
  install_weighted_polca = function(force=FALSE) {
    has_weights<-"weights" %in% names(formals(poLCA::poLCA))
    if (!has_weights | force) {
      polca_src<-here::here("external","poLCA")
      install.packages(polca_src, repos=NULL, type="source")
      message("已從external/poLCA子模組安裝支援weights之poLCA；若原poLCA已載入請重啟R session")
    }
    invisible("weights" %in% names(formals(poLCA::poLCA)))
  },
  # 第六-1部份：LCA latent variables 潛在類別模式統獨傾向
  # weights: 傳入抽樣權重欄位名（如"myown_wr"）或數值向量即用加權pseudo-ML估計
  #          （需external/poLCA子模組版，先執行install_weighted_polca()）；NULL則為原不加權估計
  custom_generate_LCA_model = function(X, n_latentclasses=3, nrep=30, maxiter=1000, modelformula=NA, firstlcaneed=c(), secondlcaneed=c(), weights=NULL, ..., exportlib=c("base"), exportvar=c(), outfile="") {
    #X此時就形同是一個問卷的dataset
      ...
      switch(
        ...
        "FALSE"=preserve_warning_tryCatch({
          polcaargs<-list(
            ...
            nclass = n_latentclasses,
            nrep = nrep,
            maxiter = maxiter
          )
          if (!is.null(weights)) {
            if (!("weights" %in% names(formals(poLCA::poLCA)))) stop("目前載入之poLCA不支援weights，請先執行install_weighted_polca()並重啟R session")
            polcaargs$weights<-weights
          }
          do.call(poLCA, polcaargs)}
        )
      )
    ...
    poLCAresult$nclass <- n_latentclasses
  ...
  repeatloadsavedestfile = function(loadsavemode="load",destfile=file.path(dataset_file_directory,"rdata","list_of_degree_of_freedom.RData"), savedestobject=list_of_degree_of_freedom, breakafterntimes=10) {
    ...
    repeat{
      ...
      if ((loadsavemode=="save" & gtools::invalid(testprocess[[1]])) | breakafterntimes==loop_times) {
  ...
  repeat_connectdb_readtable_close = function(drv, dbname, ..., tablename, breakafterntimes=10) {
  ...
  get_general_indepence_arguments_df = function(imps=self$imps, detectedcores=self$detectedcores) {
    lcaneed_independence_attitude<-self$lcaneed_independence_attitude
    lcaneed_party_constituency<-self$lcaneed_party_constituency
    lcaneed_ethnicity<-self$lcaneed_ethnicity
    lcaneed_identity<-self$lcaneed_identity
    lcaneed_other_cov<-self$lcaneed_other_cov
    lapply(names(lcaneed_independence_attitude), function(needsurvey) {
  ...
  # 迴圈開始處：對每個問卷、每個imputed dataset搜尋LCA模型組合並把結果寫入資料庫 --------------------------------
  #check at http://localhost/scripts/phpscripts/list_of_degree_of_freedom_counts.php
  run_lca_model_search = function(survey_data_imputed, idx_process_ratio=0, general_nreps=c(1, 35), general_maxiters=c(1, 1200)) {
    stopifnot(!is.null(self$dbconnect_info))
    dbconnect_info<-self$dbconnect_info
    detectedcores<-self$detectedcores
    imps<-self$imps
    lcaneed_independence_attitude<-self$lcaneed_independence_attitude
    custom_generate_LCA_model<-self$custom_generate_LCA_model
    repeat_connectdb_readtable_close<-self$repeat_connectdb_readtable_close
    general_indepence_arguments_df<-self$get_general_indepence_arguments_df()
...
###LCAmodel_with_indp_covparty使用範例（僅保留參考用）
