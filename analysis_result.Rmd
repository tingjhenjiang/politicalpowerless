...
* 族群（自變項）：三個問卷的問題皆有共同的「父母親是哪裡人」的兩個選擇題及開放填充題，我將此題目依據答案重新編碼為父母親分別為台灣閩南人、台灣客家人、大陸各省市、台灣原住民、外裔或原國籍為外籍或原國籍中國大陸的新移民、前述分類以外的臺灣人（例如有部分受訪者的父母親是「外省人第二代」，而在開放填充題中回答臺灣人）。接著依照廣義的原生論（primordialism），只要父母有其一為人口比例較少的較少數族群者，則一律視為較少數族群。
...
首先發現一個值得注意的現象。選民影響力與國會議員行為研究間關係的始祖{Miller, 1963 #12740}發現，民意在一定程度上會影響民意代表的行為。但在本研究發現顯示，在這與2016年8月至2017年5月兩段期間中，民意是否佔多數對於立法委員是否回應民意而言，影響程度卻是顯著不多。2010年7月至2011年6月（以下簡稱第七屆研究範圍期間）立委的行為與多數民意甚至呈現顯著方向相反的影響，2016年8月至2017年5月（以下簡稱第九屆研究範圍期間）立委的行為則是顯著跟著民意多數走。但是，即便上述效果不同，同樣當立法委員意向越傾向政黨（stronger partisanship）時，越傾向不回應民意。（立法委員回應民意與民意多數、政黨意見多數間關係分析表；report其他統計指標）

```{r echo=FALSE}
testdata.kmt.ruling<-filter(glmdata,term==7)
testdata.dpp.ruling<-filter(glmdata,term==9)
testdata.kmt.ruling.noignore<-filter(testdata.kmt.ruling,respondopinion!=1) %>%
  mutate_at("respondopinion",funs(factor))
testdata.dpp.ruling.noignore<-filter(testdata.dpp.ruling,respondopinion!=1) %>%
  mutate_at("respondopinion",funs(factor))
  
model_influce_from_p_p.k.2 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_nation,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)
model_influce_from_p_p.k.3 <- polr(respondopinion~opinion_pressure_from_constituent_by_nation,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)
model_influce_from_p_p.k.4 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_electionarea,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)
model_influce_from_p_p.k.5 <- polr(respondopinion~opinion_pressure_from_constituent_by_electionarea,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)


model_influce_from_p_p.d.2 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_nation,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
model_influce_from_p_p.d.3 <- polr(respondopinion~opinion_pressure_from_constituent_by_nation,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
model_influce_from_p_p.d.4 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_electionarea,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
model_influce_from_p_p.d.5 <- polr(respondopinion~opinion_pressure_from_constituent_by_electionarea,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
...
為了探究這種與一般直覺認為民意代表會回應民意相左的原因，此處先以區分時期、區分政黨回應民意的情形，以箱型圖觀察如下：

```{r}
testdata.kmt.ruling %>%
  filter(!is.na(respondopinion)) %>% ggplot(aes(x=respondopinion, y=opinion_pressure_from_constituent_by_nation)) + labs(title = "第七屆研究範圍期間立法委員回應民意與全國多數民意比例間關係") + facet_grid(party ~.) + geom_boxplot()

testdata.dpp.ruling %>%
  filter(!is.na(respondopinion)) %>%
  ggplot(aes(x=respondopinion, y=opinion_pressure_from_constituent_by_nation)) + labs(title = "第九屆研究範圍期間立法委員回應民意與全國多數民意比例間關係") + facet_grid(party ~.) + geom_boxplot()
  
testdata.kmt.ruling %>%
  filter(!is.na(respondopinion)) %>%
  ggplot(aes(x=respondopinion, y=opinion_pressure_from_constituent_by_electionarea)) + labs(title = "第七屆研究範圍期間立法委員回應民意與立法委員選區多數民意比例間關係") + facet_grid(party ~.) + geom_boxplot()

testdata.dpp.ruling %>%
  filter(!is.na(respondopinion)) %>%
  ggplot(aes(x=respondopinion, y=opinion_pressure_from_constituent_by_electionarea)) + labs(title = "第九屆研究範圍期間立法委員回應民意與立法委員選區多數民意比例間關係") + facet_grid(party ~.) + geom_boxplot()
...
```{r, echo=FALSE}
model_influce_from_p_p_s.k.2 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_nation+seatsgaptorulingparty,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)
model_influce_from_p_p_s.k.3 <- polr(respondopinion~opinion_pressure_from_constituent_by_nation+seatsgaptorulingparty,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)
model_influce_from_p_p_s.k.4 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_electionarea+seatsgaptorulingparty,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)
model_influce_from_p_p_s.k.5 <- polr(respondopinion~opinion_pressure_from_constituent_by_electionarea+seatsgaptorulingparty,
             data=testdata.kmt.ruling.noignore, Hess=TRUE)

model_influce_from_p_p_s.d.2 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_nation+seatsgaptorulingparty,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
model_influce_from_p_p_s.d.3 <- polr(respondopinion~opinion_pressure_from_constituent_by_nation+seatsgaptorulingparty,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
model_influce_from_p_p_s.d.4 <- polr(respondopinion~percent_of_same_votes_from_same_party+opinion_pressure_from_constituent_by_electionarea+seatsgaptorulingparty,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
model_influce_from_p_p_s.d.5 <- polr(respondopinion~opinion_pressure_from_constituent_by_electionarea+seatsgaptorulingparty,
             data=testdata.dpp.ruling.noignore, Hess=TRUE)
```
...
以上分析表示民意的強度（人數）並不當然能夠影響到民意代表的行為。
