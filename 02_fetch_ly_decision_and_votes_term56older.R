...
all_legislators <- openxlsx::read.xlsx(paste0(dataset_file_directory, "legislators.xlsx"), sheet = 1, detectDates = TRUE) %>%
  ...
  dplyr::filter(term %in% c(5,6)) %>%
  dplyr::distinct(term, name, onboardDate, party, leaveDate)
...
myown_vote_record_detailed_part_df<-dplyr::filter(myown_vote_record_detailed_part_df,!is.na(legislator_name)) %>%
